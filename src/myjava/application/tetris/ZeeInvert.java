...
public class ZeeInvert extends Shape
{
	ZeeInvert(Grid g)
		{
		super();
		...
		try {
			switch(getCurrentFrame()) {
				case 1:
					...
					if(b[0].getXIndex() > 0 && b[2].getXIndex() > 0
		...
		try {
			switch(getCurrentFrame()) {
				case 1:
					...
					if(b[1].getXIndex() < g.getAX() - 1 && b[3].getXIndex() < g.getAX() - 1 
					&& g.mapArray[b[1].getYIndex()][b[1].getXIndex() + 1] == 0
					&& g.mapArray[b[3].getYIndex()][b[3].getXIndex() + 1] == 0) {
						for(i = 0; i < getNumBlocks(); i++) 
							b[i].setXIndex(b[i].getXIndex() + 1);
		...
		try {
			switch(getCurrentFrame()) {
				case 1:
					...
					if(b[0].getXIndex() > 0 && b[2].getYIndex() < g.getAY() - 1
